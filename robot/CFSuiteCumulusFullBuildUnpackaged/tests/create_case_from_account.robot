...
Create Case From Account With Map Location And Validation
    ...
    # Query for an existing Person Account to use (Person Accounts have the Create Case button)
    ...
    Run Keyword If    ${account_count} == 0    Fail    No existing Person Accounts found in org. Please create a Person Account first.
    ...
    Sleep    5s
    Capture Page Screenshot    account_detail_page.png

    # Click "Create Case" button on account page
    Log To Console    Looking for Create Case button on account page...
    ${create_case_button} =    Set Variable    xpath=//button[contains(text(), 'Create Case')]
    Wait Until Element Is Visible    ${create_case_button}    timeout=15s
    Log To Console    Create Case button found - clicking it

    Click Element    ${create_case_button}

    # Wait for custom case creation page to load
    Sleep    15s
    Capture Page Screenshot    new_case_page_from_account.png
...
Validate Case
    ...
    [Documentation]
    ...  Validates that the case has the correct address, due date, AccountId,
    ...  and category fields populated from the selected options.
    ...
    # Validate via API - check all fields
    &{case} =    Salesforce Get    Case    ${case_id}

    # Validate Address field is populated
    ...
    # Validate AccountId is set to the expected account
    Should Be Equal As Strings    ${case}[AccountId]    ${expected_account_id}    msg=AccountId should be '${expected_account_id}'
    Log To Console    AccountId: ${case}[AccountId] ✓
    ...
    Should Be Equal As Strings    ${case}[cfsuite1__cfReq_Request_Sub_Reason__c]    Keeping Excess Dogs    msg=Request Sub Reason should be 'Keeping Excess Dogs'
